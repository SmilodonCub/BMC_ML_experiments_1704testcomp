...
function LandoltC_TextFile( pixperdeg, target_eccentricity, shift_angle, target_rgb, cue_size, cue_gap)
...
Landolt_C = makeLandolt_C( pixperdeg, cue_gap, cue_size, [ 1 0 0 ] );
...
%count = 0;
for l = 1 : ( size( target_rgb, 1 ) )
    for k = 1 : ( length( target_eccentricity ) )
        for j = 1 : num_positions
            target_string = ['Sqr(' num2str( sqr_radius ) ',[' num2str( target_rgb( l,1 ) ) ' ' num2str( target_rgb( l,2 ) ) ' '...
                num2str( target_rgb( l,3 ) ) '],' num2str( sqr_fill ) ',' vec_position{ mod( j,4 ) + 1 }];
            Landolt_C_tag = [ 'LC' num2str( mod( j,4 ) + 1 ) ];
            landolt_C_string = [ 'Pic(' Landolt_C_tag ',0 ,0)' ];
            distractor1_string = ['Sqr(' num2str( sqr_radius ) ',[' num2str( target_rgb( l,1 ) ) ' ' num2str( target_rgb( l,2 ) ) ' '...
                num2str( target_rgb( l,3 ) ) '],' num2str( sqr_fill ) ',' vec_position{ mod( j + 1,4 ) + 1 }];
            distractor2_string = ['Sqr(' num2str( sqr_radius ) ',[' num2str( target_rgb( l,1 ) ) ' ' num2str( target_rgb( l,2 ) ) ' '...
                num2str( target_rgb( l,3 ) ) '],' num2str( sqr_fill ) ',' vec_position{ mod( j + 2,4 ) + 1 }];
            distractor3_string = ['Sqr(' num2str( sqr_radius ) ',[' num2str( target_rgb( l,1 ) ) ' ' num2str( target_rgb( l,2 ) ) ' '...
                num2str( target_rgb( l,3 ) ) '],' num2str( sqr_fill ) ',' vec_position{ mod( j + 3,4 ) + 1 }];
