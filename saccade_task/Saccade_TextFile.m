...
function Saccade_TextFile( target_eccentricity, num_positions, angle_range, shift_angle, target_rgb )
...
%       3) angle_range. degrees. max angle from 0 to space target positions
%       across
%       4) shift_angle. degrees. this will shift the target position by a
%       set amount
%       5) target_rgb. any [ m, 3 ] size matrix


%task specific
shift_angle = shift_angle * (2*pi) / 360;
angle_range = angle_range * (2*pi) / 360;
task_folder = 'C:\monkeylogic\Experiments\saccade_task';
timing_file = 'saccade_task';
