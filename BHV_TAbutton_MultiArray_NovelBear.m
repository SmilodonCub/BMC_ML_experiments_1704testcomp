...
% 
% _  _ ____ _  _ ____ _    ___  ____ ____ ____     
% |\ | |  | |  | |___ |    |__] |___ |__| |__/     
% | \| |__|  \/  |___ |___ |__] |___ |  | |  \
%                                                                                     
%                                                    
...
saccade_rxn_time = NaN(length(BHV.TrialNumber),11);
num_saccades = zeros(30,2);
...
for ii = 1:size(BHV.AnalogData,2)                                           %for every trial 
    ...
    %--------------------------------------------------------------------------
                                                                            ...
                                                                            % (6:end)pic randomD
    fixationObjectSplit = regexp( BHV.TaskObject{BHV.ConditionNumber(ii),...
        3}, '[(,)]', 'split');                                              %need to check if this was cued or uncued
    if fixationObjectSplit{1}(1) == 'p' 
        saccade_rxn_time(ii,11) = 1;
    elseif fixationObjectSplit{1}(1) == 'S'
        saccade_rxn_time(ii,11) = 0;
    end
    ...
    for jj = 1:trialArraySize                                               %for each TaskObject in this file
       ...
       if split{1}(1) == 'p'                                                % pic & Sqr have X,Y coordinates in different fields
           ...
           if ~isempty(cell2mat(strfind(split(2),novelTarget)))             %this bit here checks to see if the current
                                                                            %trial the novel bear in it
               disp('found novel target')
               saccade_rxn_time(ii,10) = saccade_rxn_time(ii,10) + 1;
           elseif isempty(cell2mat(strfind(split(2),novelTarget)))
               saccade_rxn_time(ii,10) = saccade_rxn_time(ii,10) + 0;
           end
    ...
    if BHV.TrialError(ii) == 0                                              %if the current trial is correct, do all these things
        ...
        haztarget = BHV.InfoByCond{BHV.ConditionNumber(ii)}.ifBear;           %have to index InfoByCond by ConditionNumber or it all goes to hell
        ...
        for kk = 1:(size(A,1)-1)                                            %for every fixation detected by EyeMMV
            ...
            for mm = 1:size(pos,1)                                          %for every TaskObject for this particular trial
                ...
                if (A(kk+1,1) - pos{mm,1})^2 + (A(kk+1,2) - pos{mm,2})^2 <= targetWindowRadius^2%... 
        ...
        for rr = 1:length(pos(3:end,2))
            tag = char(pos{2+rr,3});
            text(cell2mat(pos(2+rr,1)),cell2mat(pos(2+rr,2)),tag(7:end),...
        ...
        if haztarget == '1'
            ...
            tartag = char(pos{2,3});
            %tartag = [ 'B\_' tartag(3:end) ];
            text(cell2mat(pos(2,1)),cell2mat(pos(2,2)),tartag(7:end),...
                'HorizontalAlignment','Right','VerticalAlignment','Top',...
                'FontSize',12,'Color','k')
            ...
            if size(A,1) <= 2                                               %is there were only 2 fixations & the trial was a correct one 
                ...
                saccade_rxn_time(ii,3) = size(A,1);
        ...
        elseif haztarget == '0'
            ...
            dtag = char(pos{2,3});
            text(cell2mat(pos(2,1)),cell2mat(pos(2,2)),dtag(3:end),...
                'HorizontalAlignment','Right','VerticalAlignment','Top',...
                'FontSize',12,'Color','k')
            text(A(:,1),A(:,2),num2str(A(:,7)),'HorizontalAlignment',...
                'Left','VerticalAlignment','Bottom','FontSize',12,'Color','b')
...
%         end
        ...
        bar(1:size(num_saccades,1), num_saccades,'stacked', 'barwidth', 1)
