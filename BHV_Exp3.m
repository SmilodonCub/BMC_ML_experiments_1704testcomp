...
%Reaction times of correct target present trials with immediate fixations

%Ratio of multiple array elements viewed : immediate fixations
%Average number of array elements viewed for trials with multiple array
%element fixations

%Ratio of correct target present : target absent trials
...
immediate_saccade_rxn_time = NaN(length(BHV.TrialNumber));
...
for i = 1:size(BHV.AnalogData,2)%num_correct 
    ...
    if BHV.TrialError(i) == 0
        ...
        if hazbear == '1'
            ...
            if size(A,1) <= 2
                immediate_saccade_rxn_time(i) = rxn_time(i,1);
            elseif size(A,1) > 2
...
text(8,20,[ 'Bear: ' num2str(viewed_distractors_Bear) '   Total: ' num2str(sum(num_saccades(:,1))) ])
text(8,10,[ 'NO Bear: ' num2str(viewed_distractors_NOBear) '   Total: ' num2str(sum(num_saccades(:,2))) ])
title('Number of saccades Bear/NoBear trials')
...
filename = [ 'C:\monkeylogic\Experiments\categorical_search\specific_search\BHV_VSSabs_' BHV.DataFileName '.txt' ];

WriteBHV2txt_VSS2016abs( filename , BHV, num_saccades, immediate_saccade_rxn_time )
