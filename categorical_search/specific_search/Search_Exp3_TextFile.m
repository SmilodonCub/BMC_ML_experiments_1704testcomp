...
function Search_Exp3_TextFile( target_eccentricity, num_conditions, fix_size, cue_size, image_size, num_distract, percent_nobear, target_index_ids )
    ...
    for j = 1:num_conditions
        ...
        while d <= num_distract %do the same as above for as many additional
            ...
            if ~strcmp(d_name, target_name) || ~ismember( array_entry, array_cellarray )
                ...
                Condition_cell_array( ( j + 1 + (jj-1)*num_conditions), (9 + d) ) = { distractor_string };
                d = d + 1;
            end
        end
        ...
        if jj == 1
            Condition_cell_array( ( j + 1 + (jj-1)*num_conditions), 1:9 ) = { j+ (jj-1)*num_conditions, info_string ,1, 1, timing_file, sound_string, fix_cue, fixation2, target_string };
        else
            Condition_cell_array( ( j + 1 + (jj-1)*num_conditions), 1:9 ) = { j+ (jj-1)*num_conditions, info_string ,1, 2, timing_file, sound_string, fixation, fixation2, target_string };
        end
