...
%(  ____ \(  ___  )(  ____ \(  ____ \(  ___  )(  __  \ (  ____ \(  ___  )|\     /|(  ____ \
%| (    \/| (   ) || (    \/| (    \/| (   ) || (  \  )| (    \/| (   ) || )   ( || (    \/
%| (_____ | (___) || |      | |      | (___) || |   ) || (__    | (___) || |   | || (__    
%(_____  )|  ___  || |      | |      |  ___  || |   | ||  __)   |  ___  |( (   ) )|  __)   
%      ) || (   ) || |      | |      | (   ) || |   ) || (      | (   ) | \ \_/ / | (      
%/\____) || )   ( || (____/\| (____/\| )   ( || (__/  )| (____/\| )   ( |  \   /  | (____/\
%\_______)|/     \|(_______/(_______/|/     \|(______/ (_______/|/     \|   \_/   (_______/
%                                                                                        
%                                                       
%                                                                                 
%   .m script to generate the fields for a .txt conditions file for the saccade
%   average behavioral search task in monkeylogic. for this task, 
...
%   3) on some trials (100-percent_randompos)%, target & distractor images
%   are distributed across 4 evenly spaced locations (90deg). these
%   postitions are not fixed with relation to the screen coordinates, but
%   randomly vary from trial to trial.
%   4) on other trials (percent_randompos)%, the target locations are not
%   evenly space, but vary randomly in spacing from trial to trial (care is
%   taken so that images do not overlap).
...
%   three posible locations. the subject has max_reaction_time 
...
function TBear_SaccadeAve_TextFile( target_eccentricity, num_conditions, cue_size, fix_size, image_size, num_distract, percent_nobear, percent_randompos, target_index_id )
...
%       7) num_distract int N of 3 possible distractor positions to fill
%       8) percent_nobear percentage of trials with no target
%       present(subject must return to fixation to recieve reward)
%       9) percent_randompos percentage of trial with target locations
%       randomly chosen
%       10) target_index_id --> bear target to be used consistently untill
...
%INDEXES TO SET UP EVEN/RANDOM & BEAR/NOBEAR
%eve/random == (4 even spaced (90deg) target,distractor locations)/(4
%randomly distributed target,distractor positions)
%target/no target == is there a bear (fixate bear)? or not (return to
%fixation)
%all conditions
index = 1:num_conditions;
%num even/random
num_evenpos = num_conditions - floor(num_conditions*percent_randompos/100);
num_randompos = num_conditions - num_evenpos;
%num bears/nobears
num_nobears = floor(num_conditions*percent_nobear/100);
num_bears = num_conditions - num_nobears;
%bears/nobears
nobears_index = randi( stream, num_conditions, [ num_nobears, 1 ] );
bears_index = setdiff( index, nobears_index )';
%subset indexes
num_nobears_even = floor( num_evenpos/2 );
num_bears_even = num_evenpos - num_nobears_even;
nobears_even = randsample( stream, nobears_index, num_nobears_even );
nobears_random = setdiff( nobears_index, nobears_even );
bears_even = randsample( stream, bears_index, num_bears_even );
bears_random = setdiff( bears_index, bears_even );
%even/random
even_index = cat(1, nobears_even, bears_even);
random_index = cat(1, nobears_random, bears_random);

%shift angle for randomly angled 'even' conditions
evenpos_shiftang = randi( stream, 90, [ num_evenpos, 1 ] ); %shift_angle values for TargetPositionString
...
for j = 1:num_conditions
    ...
    if ismember( j, even_index )
        ...
        if ~ismember( j, nobears_index )
            info_string = '''ifBear'', ''1'',''evenpos'',''1''';
            ...
            target_string = [ 'pic(' target_tag ',' char( target_pos_str( tindx )) ',' num2str(size_pixels) ',' num2str(size_pixels) ')'];
    ...
    elseif ismember( j, random_index )
        %pick 4 random positions that do not overlap
        target_pos_str = Random4PositionString( stream, target_eccentricity, target_angle );
                if ~ismember( j, nobears_index )
            info_string = '''ifBear'', ''1'',''evenpos'',''0''';
            tindx = randi([ 1 num_positions ]);
            condition_position([tindx]) = [];
            target_name = char( T( target_index_id ) );
            target_tag = target_name( 1:( length( target_name ) - 4 ) );
            ...
            Condition_cell_array( ( j + 1 ), 1:9 ) = { j ,info_string, 1, 1, timing_file, sound_string, fixation, fixation2, target_string };
        elseif ismember( j, nobears_index )
            info_string = '''ifBear'', ''0'',''evenpos'',''0''';
            dindx = randi([ 1 num_positions ]);
            condition_position([dindx]) = [];
            d1_name = char( D( distractor_index( j,1 ) ) );
            d1_tag = d1_name( 1:( length( d1_name ) - 4 ) );
