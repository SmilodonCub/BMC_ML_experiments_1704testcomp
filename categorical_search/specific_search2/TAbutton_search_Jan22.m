...
max_reaction_time = 1700;    %max time after fixation given to complete a saccade
hold_target_time = 1000;     %hold fixation on bear_image to get reward
fix_return_time = 100;      %if no bear target is present, this is the interval of time the subject must hold fixation for reward
...
elseif targetcon == 0
    ...
    while array_time < ( arrayon_time + max_reaction_time )
        ...
        ontarget = eyejoytrack('acquirefix', TAbutton, TAbutton_window, max_reaction_time);
        if ~ontarget, 
            eventmarker(15);
        ...
        % hold target then reward
        ontarget = eyejoytrack('holdfix', TAbutton, TAbutton_window, fix_return_time);
